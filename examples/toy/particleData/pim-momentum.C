{
    ...
    //lambda function that will reconstruct the components of the 4-momenta of the proton using the p,theta,phi variables
    ...
    //DEFINE a lambda function that will take the pip 4-momentum/proton 4-momentum and use conservation laws to give a prediction on the pims 4-momentum
    ...
    auto calculate_4mom= [&m1](ROOT::Math::PxPyPzEVector p1 ,ROOT::Math::PxPyPzEVector p2)
    {
        ...
        //using conservation of 4-momentum to get 4-momenta of the particle
    ...
    //DEFINE lambda function to output the properties of the pim from it's 4-momentum
    ...
    auto full_rec_data = Rec_data.Define("pim_4mom", calculate_4mom, {"pipRec_4mom","pRec_4mom"}).Define("calcP",P,{"pim_4mom"}).
                            Define("calcTheta", theta, {"pim_4mom"}).Define("calcPhi", phi, {"pim_4mom"}).
                            Define("pimMass", mass, {"pim_4mom"}).Define("pimM2", mass_squared, {"pim_4mom"});

    //auto pip_tru_data = Tru_data.Define("pim_4mom", calculate_4mom, {"pipTru_4mom","pTru_4mom"});

    auto full_tru_data = Tru_data.Define("pim_4mom", calculate_4mom, {"pipTru_4mom","pTru_4mom"}).Define("calcP", P, {"pim_4mom"}).
                            Define("calcTheta", theta, {"pim_4mom"}).Define("calcPhi", phi, {"pim_4mom"}).
                            Define("pimMass", mass, {"pim_4mom"}).Define("pimM2", mass_squared, {"pim_4mom"}); 


    //OUTPUT PIM VARIABLES TO TREE IN ROOT FILE

    //Rename columns in dataframe before outputting to root file
    return_column = [](double value){return value;};

    auto reconstructed_data = full_rec_data.Define("detP", return_column, {"recon.pimP"}).Define("detTheta", return_column, {"recon.pimTheta"}).
                                Define("detPhi", return_column, {"recon.pimPhi"}).Define("truP", return_column, {"truth.pimP"}).
                                Define("truTheta", return_column, {"truth.pimTheta"}).Define("truPhi", return_column, {"truth.pimPhi"});

    //Output the pim variables that were calculated using the reconstructed pip and proton variables
    auto snapshotOptions = RSnapshotOptions();
    snapshotOptions.fMode = "RECREATE";
    reconstructed_data.Snapshot("rec_tree","particle-data/pim-data.root", {"truP", "truTheta", "truPhi","detP", "detTheta", "detPhi","calcP", "calcTheta", "calcPhi", "accepted_pim"},snapshotOptions);

    //Output the proton variables that were calculated using the truth pion variables to a different tree
    snapshotOptions.fMode = "UPDATE";
    full_tru_data.Snapshot("tru_tree","particle-data/pim-data.root", {"calcP", "calcTheta", "calcPhi", "accepted_pim"},snapshotOptions);


    //PLOTTING 

    //create histograms of the masses
