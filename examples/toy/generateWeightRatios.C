...
//generate_weight_ratios("pip"), could use pip,pim or proton

void generateWeightRatios(string particle_name)
{
    
    using namespace ROOT;
    using namespace ROOT::RDF;

    //Adjust particle name to fit file naming convention
    string particle;
    ...
    //Use particle to specify the filepath of the particle data and the weights generated by the DNN/BDT
    string str_particleDataFilepath = "particleData/particle-data/" + particle + "-data.root";  
    const char* particleDataFilepath = str_particleDataFilepath.c_str();

    string str_weightDataFilepath = particle + "-training-models.root";
    const char* weightDataFilepath = str_weightDataFilepath.c_str();

    std::cout << particleDataFilepath << std::endl;
    std::cout << weightDataFilepath << std::endl;

    
    //load file containing data on the particle specified
    TFile *particleFile = new TFile(particleDataFilepath, "READ");
    ...
    recTree->AddFriend("training_calculated", weightDataFilepath);
    recTree->AddFriend("training_detected", weightDataFilepath);
    recTree->AddFriend("training_truth", weightDataFilepath);
    ...
    auto columnRatio = [](float column1Value, float column2Value)
    {
        if(column2Value==0){
            ...
            return (double)column1Value/column2Value;
        }
    };
    ...
    auto reweightRatio = [](float dnnValue1, double bdtValue1, float dnnValue2, double bdtValue2)
    {
        if(dnnValue2==0 || bdtValue2==0 ){
            ...
            double ratio = (dnnValue1*bdtValue1)/(dnnValue2*bdtValue2);
            return ratio ;
        }
    };
    ...
    auto max_ratio = particleDataRatios.Max("truDetRatioReweight").GetValue();
    auto mean_ratio = particleDataRatios.Mean("truDetRatioReweight").GetValue();

    std::cout << "the maximum value in the column is: " << max_ratio << std::endl;
    std::cout << "the mean of the column is: " << mean_ratio << std::endl;  

    //Output snapshot of dataframe to root file for truth correction training

    string str_correctionDataFilepath = "truth_correction/correction-data/" + particle + "-correction-training.root";
    const char* correctionDataFilepath = str_correctionDataFilepath.c_str();

    auto snapshotOptions = RSnapshotOptions();
    snapshotOptions.fMode = "RECREATE"; 
    particleDataRatios.Snapshot("tree", correctionDataFilepath, {"truP", "truTheta", "truPhi", "detP", "detTheta", "detPhi", "calcP", "calcTheta", "calcPhi", "calcTruRatioDNN",
                                "truDetRatioDNN", "calcTruRatioReweight", "truDetRatioReweight"}, snapshotOptions);
    ...
    auto histTruDetRatioReweight = particleDataRatios.Histo1D( TH1DModel{"truth/detected", "truth/detected Reweighted",100,5,1000},{"truDetRatioReweight"});
